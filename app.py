...
def render_razorpay_modal(output_text: str):
    """Extracts dynamic Order ID & Amount cleanly without picking numbers from Order IDs."""
    if "order_" in output_text:
        st.divider()
        st.markdown("#### 💳 Instant Razorpay Checkout")
        ...
        # 2. Extract Exact Price from Audit Log
        clean_amount = 2500  # Fallback default
        
        # Priority 1: Check Audit Trail for CREATE_RAZORPAY_ORDER status
        for log in reversed(audit_log):
            if log.get("action") == "CREATE_RAZORPAY_ORDER":
                reasoning = log.get("reasoning", "")
                m = re.search(r'Creating Order for ₹?\s*([0-9,]+)', reasoning, re.IGNORECASE) or \
                    re.search(r'₹\s*([0-9,]+)', reasoning)
                if m:
                    clean_amount = int(m.group(1).replace(',', ''))
                    break
        
        # Priority 2: Fallback to chat response (excluding order_ string)
        if clean_amount == 2500:
            text_no_order_id = re.sub(r'order_[A-Za-z0-9]+', '', output_text)
            price_match = re.search(r'₹\s*([0-9,]+)', text_no_order_id)
            if price_match:
                clean_amount = int(price_match.group(1).replace(',', ''))

        amount_in_paise = clean_amount * 100

        # 3. Render Razorpay Button
        razorpay_html = f"""
        <div style="text-align:center; padding: 10px;">
            <button id="rzp-button" style="background: linear-gradient(135deg, #146ef5 0%, #084bbb 100%); color:white; padding:12px 28px; border:none; border-radius:8px; font-weight:bold; font-size:15px; cursor:pointer; box-shadow:0 4px 14px rgba(20,110,245,0.4);">
                ⚡ Pay ₹{clean_amount:,} via Razorpay
            </button>
        </div>
        <script src="https://checkout.razorpay.com/v1/checkout.js"></script>
        <script>
...
# def render_razorpay_modal(output_text: str):
#     """Extracts dynamic Order ID & Amount to render Razorpay JS Button."""
#     if "order_" in output_text:
#         st.divider()
#         st.markdown("#### 💳 Instant Razorpay Checkout")
...
#         # 2. Robust Extract Amount from Audit Trail or Output Text
#         # Look for total amount from the audit log if available, else regex match output text
#         clean_amount = 2500
#         for log in reversed(audit_log):
#             if log.get("action") == "CREATE_RAZORPAY_ORDER" and log.get("status") == "SUCCESS":
#                 reasoning = log.get("reasoning", "")
#                 m = re.search(r'₹([0-9,]+)', reasoning) or re.search(r'([0-9,]+)\s*INR', reasoning)
#                 if m:
#                     clean_amount = int(m.group(1).replace(',', ''))
#                     break

#         if not clean_amount:
#             # Fallback regex extraction from chat response text
#             amount_matches = re.findall(r'₹\s*([0-9,]+)', output_text) or re.findall(r'([0-9,]+)', output_text)
#             if amount_matches:
#                 # Find the largest numeric value (which is usually total price)
#                 amounts = [int(m.replace(',', '')) for m in amount_matches if m.replace(',', '').isdigit()]
#                 clean_amount = max(amounts) if amounts else 2500
#             else:
#                 clean_amount = 2500

#         amount_in_paise = clean_amount * 100

#         # 3. Render Razorpay Button with Dynamic Amounts
#         razorpay_html = f"""
#         <div style="text-align:center; padding: 10px;">
#             <button id="rzp-button" style="background: linear-gradient(135deg, #146ef5 0%, #084bbb 100%); color:white; padding:12px 28px; border:none; border-radius:8px; font-weight:bold; font-size:15px; cursor:pointer; box-shadow:0 4px 14px rgba(20,110,245,0.4);">
#                 ⚡ Pay ₹{clean_amount:,} via Razorpay
...
with col_chat:
    ...
    # Display Chat History (with payment modal if message contained an order)
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])
            if message["role"] == "assistant":
                render_razorpay_modal(message["content"])

    # Handle New User Input
    if user_input := st.chat_input("Ask agent to shop... (e.g., 'I want a jacket under ₹5000')"):
        st.session_state.messages.append({"role": "user", "content": user_input})
        with st.chat_message("user"):
            st.markdown(user_input)
        ...
        with st.chat_message("assistant"):
            with st.spinner("Agent evaluating catalog & guardrails..."):
                ...
                output_text = response.get("output", "")
                st.markdown(output_text)
                render_razorpay_modal(output_text)
